...
func Chat(c *gin.Context) {
	...
	// ============================================================
	// VALIDATE AGENT + MODEL RELATION
	// ============================================================

	if agentID != 0 {

		var relation models.AgentModel
		...
		if err != nil {
			log.Printf(
				"[CHAT] RELATION ERROR: Agent %d tidak memiliki model %d",
				agentID,
				aiModel.ID,
			)
			...
			c.JSON(http.StatusBadRequest, gin.H{
				"ok":    false,
				"error": "Model tersebut belum tersedia untuk agent yang dipilih.",
			})
			return
		}
	}
